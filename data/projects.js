...
export const CASE_STUDIES = {
  pbl: {
    ...
    keyInsight: "The problem wasn't slow paperwork — it was that three disconnected systems made every hand-off an error risk. Digitizing the form without fixing the data flow would have just made the friction faster.",
    featuredArtifact: {
      src: "/case-studies/PBL/Form651.mp4",
      title: "Decoupling the input experience from the output format",
      description: "Early prototypes mirrored the paper Form 651 field-for-field — and carried over all the friction along with the familiarity. The shift was designing around pre-populated, validated inputs that auto-filled from ARMS rather than asking aircrew to re-enter data that already existed. The paper form still gets generated at the end — it just no longer drives the interaction model.",
  ...
  mydocs: {
    ...
    keyInsight: "Documents were owned by units, not people — so when a service member transferred bases, their records stayed behind. Adding a sync layer would have papered over the root cause. The fix was architectural: restructure ownership so documents follow the individual, permanently.",
    featuredArtifact: {
      src: "/case-studies/MyDocs/Artifact4.png",
      title: "Surfacing urgency at the list level",
      description: "The FEF member view was designed so a service member could land on this screen and immediately know what requires action — without opening a single record. 'Action Required' and evaluation status are first-class columns, not buried in a detail drawer. This was a deliberate inversion of how the legacy system worked, where status was only visible after navigating into individual documents.",
  ...
  parkpal: {
    ...
    keyInsight: "Every competitor used the wrong data model — binary 'accessible/not accessible' filters that couldn't capture reality. A paved path with a 12% grade is technically accessible but practically unusable for a wheelchair user. The opportunity wasn't building a better app — it was building the right data type.",
    featuredArtifact: {
      src: "/case-studies/parkpal/Design+1.webp",
      title: "Accessibility as a graded score, not a checkbox",
      description: "The trail detail view was designed around the insight that accessibility exists on a spectrum. Rather than filtering trails in or out, users see a graded accessibility score alongside surface type, condition, and grade — giving them the information to make their own judgment. Structured community input fields (not freeform reviews) keep that data reliable over time.",
